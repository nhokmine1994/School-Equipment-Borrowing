...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        if ($action === 'toggle_active') {
          ...
          $whereColumn = $id !== '' ? 'ID' : 'MaThietBi';
          $whereValue = $id !== '' ? $id : $ma;
          // Dùng cột IDActive (1=ẩn, 2=hiện) — tránh FK conflict khi viết TinhTrang bằng giá trị ngoài bảng tra cứu
          $newActive = $active ? 2 : 1;
          $newStatus = $active ? 'Sẵn sàng' : 'Vô hiệu hóa';
          $sql = "UPDATE Kho SET IDActive = ? WHERE [{$whereColumn}] = ?";
          $params = array(&$newActive, &$whereValue);
          $stmt = sqlsrv_prepare($conn, $sql, $params);
          $ok = false;
          if ($stmt && sqlsrv_execute($stmt)) {
            $rows = sqlsrv_rows_affected($stmt);
