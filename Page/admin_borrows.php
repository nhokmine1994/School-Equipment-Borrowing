...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        // Helper: cập nhật trạng thái phiếu + đồng bộ kho theo đúng logic sp_DuyetMuon / sp_TraThietBi:
        //   pending → approved : TRỪ Kho.SoLuong (thiết bị đi ra khỏi kho)
        //   returned           : CỘNG lại Kho.SoLuong (phục hồi số liệu)
        //   rejected           : không đổi kho (chưa từng trừ)
        if (!function_exists('admin_borrow_set_status')) {
          function admin_borrow_set_status($conn, $borrowId, $newStatus, $hanTra = null)
          {
            if ($borrowId === '') {
              return ['ok' => false, 'error' => 'Thiếu mã phiếu mượn.'];
            }

            // 0. ĐỌC TRẠNG THÁI HIỆN TẠI TRƯỚC KHI UPDATE (để biết phiếu đã trừ/cộng kho chưa)
            $info = @sqlsrv_query($conn, "SELECT MaThietBi, SoLuong, ISNULL(TinhTrangMuon, '') AS TrangThai FROM PhieuMuon WHERE SoPhieuMuon = ?", [$borrowId]);
            if (!$info || !($phieu = sqlsrv_fetch_array($info, SQLSRV_FETCH_ASSOC))) {
              return ['ok' => false, 'error' => 'Phiếu mượn không tồn tại.'];
            }
            $maThietBi = trim((string) ($phieu['MaThietBi'] ?? ''));
            $soLuong = (int) ($phieu['SoLuong'] ?? 0);
            $trangThaiCu = trim((string) ($phieu['TrangThai'] ?? ''));
            $newStatusKey = strtolower(trim((string) $newStatus));

            // 1. Cập nhật trạng thái phiếu (cột TEXT TinhTrangMuon — schema thực tế), fallback numeric
            if (!function_exists('seb_resolve_phieu_muon_text_status_column')) {
              return ['ok' => false, 'error' => 'Thiếu helper cột trạng thái.'];
            }
            $textInfo = seb_resolve_phieu_muon_text_status_column($conn);
            if (!empty($textInfo['ok'])) {
              $label = function_exists('seb_resolve_borrow_status_label') ? seb_resolve_borrow_status_label($conn, $newStatus) : '';
              if ($label === '' || $label === null) {
                $label = (string) $newStatus;
              }
              if ($hanTra !== null && trim((string) $hanTra) !== '') {
                $sqlStatus = "UPDATE PhieuMuon SET [" . str_replace(']', ']]', $textInfo['column']) . "] = ?, HanTra = ? WHERE SoPhieuMuon = ?";
                $params = [$label, $hanTra, $borrowId];
              } else {
                $sqlStatus = "UPDATE PhieuMuon SET [" . str_replace(']', ']]', $textInfo['column']) . "] = ? WHERE SoPhieuMuon = ?";
                $params = [$label, $borrowId];
              }
              $stmt = @sqlsrv_prepare($conn, $sqlStatus, $params);
              if (!$stmt || !@sqlsrv_execute($stmt)) {
                return ['ok' => false, 'error' => seb_sql_error_message('Cập nhật trạng thái thất bại.')];
              }
            } else {
              $statusInfo = seb_resolve_phieu_muon_status_column($conn);
              $statusId = seb_resolve_borrow_status_id($conn, $newStatus);
              if (empty($statusInfo['ok']) || empty($statusId['ok'])) {
                return ['ok' => false, 'error' => $statusInfo['error'] ?? $statusId['error'] ?? 'Không xác định được trạng thái.'];
              }
              $sqlStatus = "UPDATE PhieuMuon SET [" . str_replace(']', ']]', $statusInfo['column']) . "] = ? WHERE SoPhieuMuon = ?";
              if (!(@sqlsrv_query($conn, $sqlStatus, [$statusId['id'], $borrowId]))) {
                return ['ok' => false, 'error' => seb_sql_error_message('Cập nhật trạng thái thất bại.')];
              }
            }

            // 2. Đồng bộ số liệu kho (transaction-safe) — dùng trạng thái đã đọc TRƯỚC update:
            //    pending → approved : TRỪ SoLuong (thiết bị ra khỏi kho)
            //    returned           : CỘNG lại SoLuong (phục hồi số liệu)
            //    rejected           : không đổi kho (chưa từng trừ)
            // (giữ nguyên biến $maThietBi/$soLuong/$trangThaiCu đã đọc đầu hàm)

            if ($maThietBi === '' || $soLuong <= 0) {
              return ['ok' => true];
            }

            $alreadyApproved = in_array($trangThaiCu, ['approved', 'đã duyệt', 'da duyet', 'Đang mượn', 'dang muon'], true);
            $alreadyReturned = in_array($trangThaiCu, ['returned', 'đã trả', 'da tra', '4'], true);

            $needDeduct = $newStatusKey === 'approved' && !$alreadyApproved;
            $needRestore = $newStatusKey === 'returned' && !$alreadyReturned;

            if (!$needDeduct && !$needRestore) {
              return ['ok' => true];
            }

            if (sqlsrv_begin_transaction($conn) === false) {
              return ['ok' => false, 'error' => 'Không mở được transaction kho.'];
            }

            if ($needDeduct) {
              $luongCheck = @sqlsrv_query($conn, 'SELECT SoLuong FROM Kho WHERE MaThietBi = ? AND ISNULL(IDActive, 2) = 2', [$maThietBi]);
              if (!$luongCheck || !($kho = sqlsrv_fetch_array($luongCheck, SQLSRV_FETCH_ASSOC))) {
                sqlsrv_rollback($conn);
                return ['ok' => false, 'error' => 'Không tìm thấy thiết bị đang hoạt động trong kho để trừ số lượng.'];
              }
              $soLuongCon = (int) ($kho['SoLuong'] ?? 0);
              if ($soLuongCon < $soLuong) {
                sqlsrv_rollback($conn);
                return ['ok' => false, 'error' => "Không đủ số lượng thiết bị trong kho (còn {$soLuongCon}, cần {$soLuong})."];
              }
              if (!@sqlsrv_query($conn, 'UPDATE Kho SET SoLuong = SoLuong - ? WHERE MaThietBi = ?', [$soLuong, $maThietBi])) {
                sqlsrv_rollback($conn);
                return ['ok' => false, 'error' => seb_sql_error_message('Trừ kho thất bại.')];
              }
            }
            ...
            if ($needRestore) {
              if (!@sqlsrv_query($conn, 'UPDATE Kho SET SoLuong = SoLuong + ? WHERE MaThietBi = ?', [$soLuong, $maThietBi])) {
                sqlsrv_rollback($conn);
                return ['ok' => false, 'error' => seb_sql_error_message('Phục hồi kho thất bại.')];
              }
            }

            if (sqlsrv_commit($conn) === false) {
              sqlsrv_rollback($conn);
              return ['ok' => false, 'error' => 'Commit kho thất bại.'];
            }
        ...
        if ($action === 'approve') {
          ...
          $result = admin_borrow_set_status($conn, $borrow_id, $newStatus, $ngay_tra_du_kien);
          $ok = !empty($result['ok']);
          if (!$ok) {
            $message = $result['error'] ?? 'Duyệt thất bại.';
            $messageType = 'danger';
          } else {
        ...
        if ($action === 'reject') {
          ...
          $result = admin_borrow_set_status($conn, $borrow_id, $newStatus, null);
          $ok = !empty($result['ok']);
          if (!$ok) {
            $message = $result['error'] ?? 'Từ chối thất bại.';
            $messageType = 'danger';
          } else {
        ...
        if ($action === 'returned') {
          $result = admin_borrow_set_status($conn, $borrow_id, 'returned', null);
          $ok = !empty($result['ok']);
          if (!$ok) {
            $message = $result['error'] ?? 'Đánh dấu trả thất bại.';
            $messageType = 'danger';
          } else {
