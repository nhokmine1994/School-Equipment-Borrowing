...
{
    $fallback = '../Page/kho_thiet_bi.php';
...
if(isset($_POST['login']))
{
    ...
    // Debug: log what the browser actually sent
    $debugLine = date('c') . "\tFORM_SUBMIT\turi=" . ($_SERVER['REQUEST_URI'] ?? '') . "\tget_return=" . ($_GET['return'] ?? '(none)') . "\treferer=" . ($_SERVER['HTTP_REFERER'] ?? '(none)');
    @file_put_contents(__DIR__ . '/../logs/login_debug.txt', $debugLine . "\n", FILE_APPEND | LOCK_EX);
...
{
    header('Location: ../Page/kho_thiet_bi.php');
    exit;
}
