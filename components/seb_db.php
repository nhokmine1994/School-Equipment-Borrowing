...
function seb_pick_column_name(array $columns, array $candidates, array $types = [], $strict = false)
{
    ...
    // Fuzzy type fallback may accidentally match unrelated columns (e.g. SoLuong on PhieuMuon).
    // Only allowed in non-strict mode for legacy compatibility.
    if (!empty($types) && !$strict) {
...
function seb_resolve_column_by_candidates($conn, $tableName, array $candidates, array $types = [], $excludeIdentity = false, $strict = false)
{
    ...
    $column = seb_pick_column_name($columns, $candidates, $types, $strict);
...
function seb_fetch_bo_mon_list($conn)
{
    $list = [];
    ...
    // Nguồn chính: bảng tra cứu dbo.BoMon (master list)
    $stmt = @sqlsrv_query($conn, 'SELECT TenBoMon FROM BoMon ORDER BY TenBoMon COLLATE Vietnamese_CI_AI ASC');
    if ($stmt) {
        while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
            $ten = trim((string) ($row['TenBoMon'] ?? ''));
            if ($ten !== '') {
                $list[] = $ten;
            }
        }
    ...
    // Fallback: BoMon đang tồn tại trên TaiKhoan
    $stmt2 = @sqlsrv_query($conn, "SELECT DISTINCT BoMon FROM TaiKhoan WHERE BoMon IS NOT NULL AND LTRIM(RTRIM(BoMon)) <> '' ORDER BY BoMon ASC");
    if ($stmt2) {
        while ($row = sqlsrv_fetch_array($stmt2, SQLSRV_FETCH_ASSOC)) {
            $ten = trim((string) ($row['BoMon'] ?? ''));
            if ($ten !== '') {
                $list[] = $ten;
            }
        }
...
function seb_resolve_phieu_muon_device_column($conn)
{
    return seb_resolve_column_by_candidates(
        ...
        true,
        true // strict: không fuzzy-match sang cột khác (vd SoLuong)
    );
}
...
function seb_fetch_devices_map($conn)
{
    ...
    // Danh sách thiết bị "đã thanh lý / ngưng sử dụng" (IDActive=1) — loại khỏi mọi luồng công cộng:
    // không xuất hiện trong danh sách, không thể mượn, không thêm được vào kho cá nhân.
    $liquidatedCodes = [];
    $inactiveStmt = @sqlsrv_query($conn, 'SELECT MaThietBi FROM Kho WHERE IDActive = 1');
    if ($inactiveStmt) {
        while ($row = sqlsrv_fetch_array($inactiveStmt, SQLSRV_FETCH_ASSOC)) {
            $code = trim((string) ($row['MaThietBi'] ?? ''));
            if ($code !== '') {
                $liquidatedCodes[strtolower($code)] = true;
            }
        }
    }
    ...
    while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
        ...
        $deviceCode = trim((string) ($row['MaThietBi'] ?? ''));
        if ($deviceCode !== '' && isset($liquidatedCodes[strtolower($deviceCode)])) {
            continue; // thiết bị thanh lý — không đưa vào luồng công cộng
        }
        if ($device['id'] !== '') {
...
function seb_map_kho_row_to_device(array $row, array $categoryMap = [])
{
    ...
    $rawId = trim((string) ($row['IDThietBi'] ?? $row['ID'] ?? ''));
    $rawCode = trim((string) ($row['MaThietBi'] ?? $row['IDThietBi'] ?? $row['ID'] ?? ''));

    return [
        'id' => $rawId !== '' ? $rawId : $rawCode,
        'code' => $rawCode !== '' ? $rawCode : $rawId,
...
function seb_fetch_personal_devices($conn, $taiKhoan)
{
    ...
    while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
        ...
        // Nếu $ma không còn trong devicesMap (vd: thiết bị đã thanh lý — bị loại khỏi map),
        // bỏ thủ tục fallback tay để không khiến thiết bị thanh lý lại hiện trong kho cá nhân.
        $inactiveCheck = @sqlsrv_query($conn, 'SELECT 1 FROM Kho WHERE MaThietBi = ? AND IDActive = 1', [$ma]);
        if ($inactiveCheck && sqlsrv_fetch_array($inactiveCheck, SQLSRV_FETCH_ASSOC)) {
            continue;
        }
...
function seb_create_borrow_request($conn, $taiKhoan, $deviceId, $deviceCode, $tenThietBi, $soLuong, $hanTra = null)
{
    ...
    $deviceIdColumn = seb_resolve_column_by_candidates(
        ...
        true,
        true // strict: tránh nhặt SoLuong làm device-id column
    );
    ...
    if (!empty($statusColumn['ok'])) {
        ...
        $textSameAsStatus = !empty($textStatusColumn['ok']) && strtolower(trim((string) ($textStatusColumn['column'] ?? ''))) === strtolower(trim((string) ($statusColumn['column'] ?? '')));
        if (!empty($textStatusColumn['ok']) && !$textSameAsStatus) {
        ...
        // Me защит tránh duplicate cột text-status (đã thêm qua $statusColSql/$textColKey ở trên).
        $textSameAdded = !empty($textStatusColumn['ok']) && strtolower(trim((string) ($textStatusColumn['column'] ?? ''))) === strtolower(trim((string) $textColKey));
        if (!empty($textStatusColumn['ok']) && !$textSameAdded) {
            $sqlColumns[] = '[' . str_replace(']', ']]', $textStatusColumn['column']) . ']';
            $sqlValues[] = '?';
            $params[] = $pendingLabel;
        }
